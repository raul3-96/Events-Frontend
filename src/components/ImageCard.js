...
import { EditBottom, DeleteBottom} from './Bottom'
  ...
  const renderImageCardBody = (props) => {
    return (
      <View style={styles.card} >
        ...
        <View style={props.cardBody ? props.cardBody: styles.cardBody}>
            <TextSemiBold textStyle={styles.cardTitle}>{props.title}</TextSemiBold>
            {props.children}
            {props.cardBottoms && <View style={props.cardBottoms}>
              {props.editEnable && props.editEnable == true && 
                <EditBottom  style={[{width: '50%' }]} onPress={props.onPressEdit}></EditBottom>}
              {props.deleteEnable && props.deleteEnable == true && 
                <DeleteBottom style={[{width: '50%' }]} onPress={props.onPressDelete}></DeleteBottom>}
            </View>}
        </View>
      </View>
    )
